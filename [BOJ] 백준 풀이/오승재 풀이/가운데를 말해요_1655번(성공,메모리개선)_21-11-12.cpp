...
int main()
{
	...
	int* left_heap=new int [(n+5)/2]();	
	int* right_heap=new int [(n+5)/2]();
	
	std::make_heap(left_heap,left_heap+(n+5)/2,std::less<int>());//내림차순	
	std::make_heap(right_heap,right_heap+(n+5)/2,std::greater<int>());//오름차순
